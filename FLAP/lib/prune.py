...
def compress(layer, attn_mask, mlp_mask, attn_mean_inp, mlp_mean_inp, device, bias=True, unstr=False):
    ...
    if unstr:  # Only mask, do not really prune
        ...
        if attn_mask is not None:
            ...
            print(layer.self_attn.v_proj.weight.data.shape)
...
def prune_flap(args, model, tokenizer, device=torch.device("cuda:0")):
    ...
    dataloader, _ = get_loaders("bookcorpus", nsamples=args.nsamples,seed=args.seed,seqlen=model.seqlen,tokenizer=tokenizer)
...
def prune_wanda_sp(args, model, tokenizer, device=torch.device("cuda:0")):
    ...
    print("loading calibdation data")
    dataloader, _ = get_loaders("bookcorpus",nsamples=128,seed=args.seed,seqlen=model.seqlen,tokenizer=tokenizer)
    print("dataset loading complete")

...
#     )
    ...
    for i in range(len(model.model.layers)):
        ...
        if model.model.layers[i].mlp.down_proj.bias is None:
            model.model.layers[i].mlp.down_proj.bias = torch.nn.Parameter(torch.zeros(model.model.layers[i].mlp.down_proj.out_features).to(torch.float16))
...
def main():
    ...
    parser.add_argument('--seed', type=int, default=1234, help='Seed for sampling the calibration data.')
